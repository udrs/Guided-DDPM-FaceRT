...
from ddpm.functions.ckpt_util import get_ckpt_path, download
from ddpm.functions.denoising import efficient_generalized_steps

import torchvision.utils as tvu

from ddpm.guided_diffusion.unet import UNetModel
from ddpm.guided_diffusion.script_util import create_model, create_classifier, classifier_defaults, args_to_dict
import random
...
class Diffusion(object):
    ...
    def sample(self):
        ...
        ckpt = os.path.join("/media/lscsc/nas/wendi/CodeFormerDDPM/ddpm/exp/", "logs/celeba/celeba_hq.ckpt")
        model.load_state_dict(torch.load(ckpt, map_location=self.device))
        model.to(self.device)
        model = torch.nn.DataParallel(model)
        result = self.sample_sequence(model, cls_fn)
        return result
    ...
    def sample_sequence(self, model, cls_fn=None):
        ...
        if deg[:2] == 'cs':
            ...
            blur_by = int(deg[2:])
            from ddpm.functions.svd_replacement import SuperResolution
        ...
        for x_orig, classes in pbar:
            ...
            for i in range(len(pinv_y_0)):
                # pathtest = os.path.join(self.args.image_folder, f"y0_{idx_so_far + i}.png")
                tvu.save_image(
